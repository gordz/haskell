...
-- Ex. 3
-- ===================================
--  that takes as arguments two functions f and g and a list xs, and applies f to all elements at even positions ([0, 2..]) in xs and g to all elements at odd positions ([1, 3..]) in xs
-- Example: funkyMap (+10) (+100) [1, 2, 3, 4, 5] = [(+10) 1, (+100) 2, (+10) 3, (+100) 4, (+10) 5].
funkyMap :: (a -> b) -> (a -> b) -> [a] -> [b]
funkyMap f g xs = map (funkyMap' f g ) (zip [0..] xs)

funkyMap' :: (a -> b) -> (a -> b) -> (Integer, a) -> b
funkyMap' f g pair
	| even (fst pair) = f (snd pair)
	| otherwise = g (snd pair)
